...
$i=1;
open DAT, ">".$_dat_;
print DAT "<p><table><tr><th>name</th><th>Status</th><th>Used</th><th>Size</th><th>Use\%</th><th>Graph of Used</th></tr>\n";
foreach $_ (sort(keys %nas)){
  $nasname = $_;
  $nasname =~ s/\/mnt\///g;
  open DSTAT, "/var/www/dat/".$nasname.".stat";
  $_tmp_ = <DSTAT>;
  close DSTAT;
  ($gomi,$sts,@ggomi)=split /:/,$_tmp_;
  $sts=~ s/ //g;
  chomp($sts);
  if($sts eq "PASSED"){$sts="<span style=\"color: blue;\">".$sts."</span>";}
  else{$sts="<span style=\"color: red;\">".$sts."</span>";}
  ...
  close DISPLAY;
  printf DAT "<tr><td class=name>$_</td><td class=name>$sts</td><td>$nas{$_}[0]GB</td><td>$nas{$_}[1]GB</td><td>$nas{$_}[2]\%</td><td class=graph><img src=\"./disk/df$i.png\" border=0></td></tr>\n";
  $i++;
}
print DAT "</table></p>\n";
close DAT;
